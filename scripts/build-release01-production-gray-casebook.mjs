...
const PRODUCT_COMMIT = 'ed83a095d48b75e70da1ea726607fbce3a7f176a';
...
const MR_WINDOW_END = '2026-08-27T16:20:00+08:00';
...
const RECENT_MR_CASE_MAPPING = new Map([
  ...
  ['1346', ['MRSMOKE-FAIL-001', 'MRSMOKE-AUTH-001', 'MRSMOKE-SKILL-001', 'MRSMOKE-ROUTE-001']],
...
const RECENT_MR_STATIC_AUDITS = new Map([
  ...
  ['1349', {
    ...
    disposition: 'Dashboard-only：OTA release floor 与 dashboard 版本展示静态审计；不新增桌面QWork E2E',
    reason: '保留 dashboard release-client、BFF、前端版本展示及测试文件清单；该 MR 不改变当前桌面候选的可执行行为，不计12/70/160桌面通过',
  }],
]);

// GitLab API 在 2026-08-27 16:15 CST 以单次只读请求冻结的增量 MR 文件清单。
// deepbankV2 保持只读；生成器用上一 Casebook 可读提交重建旧 35 条，再追加这 21 条。
...
function patchSmokeCase(testCase) {
  ...
  next['备注'] = `${asString(next['备注'])}；本窗口56个直接合入MR已在“近2天MR覆盖”逐条映射，Dashboard/CI/eval/refactor/version-only变更只做静态合同审计。`;
...
async function main() {
  ...
  if (mrRows.length !== 56) throw new Error(`本窗口直接合入MR必须恰好56个，actual=${mrRows.length}`);
  ...
  addSheet(workbook, '源码依据', 'Casebook源码与审计依据', '所有依据均绑定固定commit；deepbankV2仓库只读，QbotTestAgent负责Case、执行器、证据和放行规则。',
    ['类型', '位置/版本', '用途', '校验'], [
      ...
      ['上一Casebook产品基线', PREVIOUS_CASEBOOK_PRODUCT_COMMIT, '冻结21个新增直接合入MR的审计起点', '本地只读提交可达；增量文件清单由单次GitLab只读请求冻结'],
      ['MR窗口基线', MR_WINDOW_BASELINE_COMMIT, '重建2026-08-24起全部56个直接合入MR', `git log ${MR_WINDOW_BASELINE_COMMIT}..${PREVIOUS_CASEBOOK_PRODUCT_COMMIT} --first-parent --merges + 21条只读增量`],
