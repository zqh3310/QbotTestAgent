#!/usr/bin/env bash
# QA-only helper: restart the already-built local QBot control plane with a
# runner-owned SkillHub URL. Electron and deepbankV2 source files stay intact.
set -euo pipefail
...
for _ in {1..160}; do
  if curl -fsS "http://localhost:$PORT/api/health" >/dev/null 2>&1; then
    printf 'control plane restarted pid=%s skillhub=%s\n' "$server_pid" "${SKILLHUB_URL:-<empty>}"
    exit 0
  fi
