...
const required = [
  ...
  ['QWork 0.0.17 统一加号菜单用稳定 section/option testid 与 Radix 键盘语义', /UNIFIED_COMPOSER_SUBMENUS[\s\S]*section: 'mode'[\s\S]*composer-plus-sub-mode[\s\S]*composer-plus-section-\$\{config\.section\}[\s\S]*row\.press\('ArrowRight'\)[\s\S]*composer-work-mode-\$\{mode\}[\s\S]*workModeSelectionSnapshot[\s\S]*composer-work-mode-chip/],
