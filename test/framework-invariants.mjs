...
const required = [
  ...
  ['附件原生弹窗使用固定宿主 node-screenshots 且截图后自动取消截图层', /(?=[\s\S]*prepareTeamsNativeDialogCapture[\s\S]*managedFixtureLoopbackOrigin\(upstreamCdpUrl\))(?=[\s\S]*lib-screencapture-app\/dist\/electron\.html)(?=[\s\S]*screenshots\.on\('capture')(?=[\s\S]*screenshots\.cancel\(\))(?=[\s\S]*ipcRenderer\.invoke\('screenshotStart', false\))/],
  ['附件原生弹窗绕开第二个 Browser 级 CDP 附加并直连受管 page target', /(?=[\s\S]*managedTeamsPageTargetWebSocket[\s\S]*\/devtools\\\/page\\\/)(?=[\s\S]*readManagedTeamsCdpTargets[\s\S]*\/json\/list)(?=[\s\S]*openManagedTeamsPageTarget[\s\S]*new WebSocket\(websocketUrl\))(?=[\s\S]*Runtime\.evaluate)/],
  ['附件原生截图必须先于 OK、绑定同一弹窗文案并校验完整 PNG', /(?=[\s\S]*stageAttachmentPathsThroughComposer[\s\S]*nativeCapture\.capture\(nativeFile, dialogMessage\)[\s\S]*await dialog\.accept\(\))(?=[\s\S]*capture_completed_before_accept)(?=[\s\S]*0x89, 0x50, 0x4e, 0x47)(?=[\s\S]*teams-native-node-screenshots)(?=[\s\S]*capturedDialogMessage)/],
