...
const required = [
  ...
  ['附件原生弹窗在 alert 前排队官方主进程截图且保持原始 alert 语义', /installRendererDialogCaptureHook[\s\S]*window\.agent\.shell\.captureViewport\(\)[\s\S]*originalAlert\.call\(window, message\)[\s\S]*stageAttachmentPathsThroughComposer[\s\S]*await installRendererDialogCaptureHook\(page\)/],
  ['附件原生弹窗主进程截图绑定同一提示文案并校验 PNG 后才作为证据', /collectRendererDialogCapture[\s\S]*String\(capture\.message\) === String\(expectedMessage[\s\S]*0x89, 0x50, 0x4e, 0x47[\s\S]*qwork-shell-captureViewport-dialog-hook[\s\S]*capturedDialogMessage/],
