...
- `BETA-FILE-002` 必须上传互异的 `qbot-image-flow.png` 与 `qbot-image-risk.png`，真实删除并恢复其中一张，最终 Composer 仍精确包含两张图片后才能发送。回复 Oracle 必须同时命中 `QBot Release Flow` 的 INPUT/ANALYZE/DELIVER 与发布证据门禁，以及 `Release Risk Matrix` 的 IMPACT/PROBABILITY 和 P0/P1/P2 锚点；轴名允许标准中文等价词“影响/影响程度”和“概率/可能性”，P0/P1/P2 与对应风险标签之间允许空白、项目符号、冒号、短横线和 Markdown 标记等常见等价格式，但两组轴和三组风险标签仍须全部精确命中。只上传或只识别一张图片不得产生可信产品结论。
