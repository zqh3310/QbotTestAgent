...
| QWork 核心生命线门禁 | `PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-05-r15.xlsx` | `核心生命线门禁` | 16 | `8523a10715a384f0d321f468a5350b393f19832008f585731fe83e292982ff2a` |
| 生产灰度发布 | `PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-05-r15.xlsx` | `生产灰度门禁Case` | 70 | `8523a10715a384f0d321f468a5350b393f19832008f585731fe83e292982ff2a` |
| 全量正常功能回归 | `PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-05-r15.xlsx` | `全量功能回归Case` | 160 | `8523a10715a384f0d321f468a5350b393f19832008f585731fe83e292982ff2a` |
...
文件名与 SHA 均精确匹配本节 r15 合同的正式 Casebook、十字段发布身份，以及分别通过
`--release-intake-g1`、`--release-intake-g2`、`--release-intake-g3`、
`--release-intake-g4` 提供的四份强制 `READY` intake。四份报告路径必须互不相同，分别
精确绑定 G1-G4 对应 Sheet 和有序 Case ID，同时 release ref、HEAD、仓库和设计基线必须
全等；调用者还必须独立提供
...
全部四份 intake 路径不同，固化来源、时间、仓库、ref、HEAD 和文件 SHA；本地 Git 来源还要以
...
fail-closed；错误只报告稳定类别，不得回显未知参数的内联值、后随值或疑似 secret。
...
`release-test-integrity.json` 和 `events/*.json`；plan 使用明确不兼容的 `v3` schema，
state/integrity/event 使用 `v2` schema，旧 plan v2、旧 v1 控制树及单 intake 控制树均不得
静默续跑。所有命令必须持有 macOS `lockf` 或同等级
...
r15 已通过排他事务正式生成并独立验收，当前唯一正式文件为
`PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-05-r15.xlsx`，
SHA-256 为 `8523a10715a384f0d321f468a5350b393f19832008f585731fe83e292982ff2a`。
生成输入为
`outputs/20260905161230_release01-r12-to-latest-r15-intake_framework-e3ed6d5_casebook-da9181f/release-intake.json`，
文件 SHA-256 为 `13bd0f57bbf6b07720b470feb708865ea7f60c75f6d55ae98a47a15940599a09`；
该 intake 在框架 `e3ed6d519e0f947185c29a04a6f0290df48371bb` 上从 r12 产品基线
`4693c5bd57b1170bed530e7559f9dc93a0b4a492` 以 `--max-commits 500` 完成 GitLab API
全量扫描，冻结 `origin/release/0.1@6d482c9ccbceb74d4ebf81610d980e5fe15def6c`，
共 37 个增量 MR、171 个总 MR，并包含 `!1573` 专用源码合同。构建目录为
`outputs/20260905162000_release01-r15-casebook-build_framework-e3ed6d5_intake-13bd0f57`；
14 个 Sheet、15 张渲染、公式错误扫描和 G1/G2/G3/G4 导出后协议/runtime 审计全部通过，
四层能力分别为 16/16、12/12、70/70、160/160，`strict_controller_required=0`、
`unsupported_runtime=0`，G4 前 70 条与 G3 精确同序。该设计 intake 的
构建审计 `casebook-build-audit.json` 的 `release_intake.execution_authorized=false` 仍然
有效；原始 `release-intake.json` 顶层不定义 `execution_authorized`。正式执行必须基于
提交推送后的最终框架 commit 重新扫描 release/0.1，并生成新的执行 intake、独立
observation 和精确 READY，不能复用本次生成输入。r14 转为历史正式基线，其历史批次和
失败证据继续永久保留。

下一版生成目标为
`PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-05-r16.xlsx`，
用于固化 `BETA-INIT-003` 的双 catalog ledger、产品动作 trace 和发送计数差分合同。当前
r16 尚未正式生成、尚无正式 SHA-256，也未完成独立验收，因此不得作为 Casebook 入口、
不得用于 intake/pretest/runner。生成器指向 r16 只表示下一次排他构建目标；在 r16 经过
新 intake、四 Sheet 同步审计、渲染验收、正式排他发布并与文档/invariant/状态机在同一
干净 pushed 基线中切换之前，本节 r15 路径和 SHA 仍是唯一正式身份。
...
截至 2026-09-09 的 R16 准备态只读 GitLab API 权威扫描已从 r12 设计基线稳定观测到
`origin/release/0.1@87b6f9110b257c79e5209c01f9f4104f658e430f`，first-parent 增量为 86 个、继承
r12 后总数为 220 个，最后一个直接合入 MR 必须是 `!1610`。权威输入为
...
该报告冻结 86 个 commit 的归因、changes 与当时已注册源码合同；其顶层结论仍为
`BLOCKED`，并记录旧工具把独立 blocking-risk 失败错误投影为 API freshness blocker，以及
`execution_runner_message_isolation_missing` blocking-risk 未通过。因此它不得用于生成正式
r16、pretest 或 runner。最新 `!1612` 必须显式绑定
`deepbankv2-mr-1552-execution-runner-isolation/v1`；只有 blocking-risk v5 三项检查及全部
successor AST 子合同均为 true 才能解锁，任一 manager、controller、cancellation、deadline、
callback settlement、event flow、message、termination 或 desktop lease 链不成立都必须继续
`NO_GO`。
...
R16 还必须按上述 first-parent 顺序追加 `!1604`、`!1603`、`!1598`、`!1600`、
`!1602`：`!1604` 映射自动化调度、活动、任务与宿主相邻回归；`!1603` 映射 Skill、
聊天、任务与宿主相邻回归；`!1598` 映射上下文压缩相关的活动、失败、聊天、任务、性能
与宿主相邻回归；`!1602` 映射初始化、宿主、Teams 首启及导航/入口相邻回归。
`!1600` 仅为静态合同，固定 merge SHA
`610295245f8aad0dbf5bf5b6b421ff285c22349d`、diff SHA-256
`7b5cd65ac8b2685d44fd46364253e8709cf8c5f344d4883757fd9d48548ab0fb` 和精确 24 个
changed paths；不得新增桌面 Case 或把 CI/治理声明冒充测试执行结果。

相对上述 `!1602` 边界，权威扫描继续按 first-parent 精确追加 11 个 MR：`!1605`、
...
`!1612` 的相邻 Case 不能替代其 G0 blocking-risk v5 源码证明；新增集合的 IID、
merge SHA、diff SHA、changed paths、源码合同和 blocking-risk 绑定任一漂移均须
fail-closed。
...
七项均须冻结 IID、merge SHA、diff bytes/SHA、GitLab labels、changed paths、空源码合同集合
和权威 impact direct Case 顺序；任一漂移均 fail-closed。静态项不得新增桌面 Case，相邻项
也不得把源码声明或产品单元测试资产冒充直接 E2E。
...
相邻链通过冒充这些内部合同已执行或通过。
`--out` 在调用前必须不存在，即使已存在目录为空也禁止复用。历史空目录
`outputs/20260905_release01_casebook_16-12-70-160-r15` 未被复用，正式生成使用上述新目录。
正式目标也必须在调用前不存在；生成器只能在 G1/G2/G3/G4 导出后协议与 runtime 能力分别
达到 16/16、12/12、70/70、160/160 后原子排他发布。任何已有目标、审计失败或中途产物都
不得覆盖既有正式 Casebook 或取得正式身份。
...
当前正式设计基线是 `origin/release/0.1@6d482c9ccbceb74d4ebf81610d980e5fe15def6c`，
...
4. 执行能力审计：
   ...
   npm run core-beta:capability-audit -- \
     --casebook PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-05-r15.xlsx \
     --sheet 核心生命线门禁 \
...
5. 执行统一真实运行前自检。以下以 360Teams 为例；所有字段必须替换为本轮冻结发布值：
   ...
   npm run core-beta:pretest -- \
     ...
     --expected-count 16 \
     --expected-sha256 8523a10715a384f0d321f468a5350b393f19832008f585731fe83e292982ff2a \
...
CASEBOOK="$PWD/PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-05-r15.xlsx"
...
- `BETA-INIT-003` 不能只凭通用 runtime/SDK/capabilities ready 或“技能运行环境已清理”文案通过。动作前先在有界窗口读取完整 `getSkillsCatalog()`；允许初始 `syncing`，但必须收敛为 `phase=before_action` 的独立 `qbot-core-beta-skill-reinstall-catalog-observations/v2` ledger。真实点击【一键重装 Skill】并完成破坏性确认后，再生成 `phase=after_action` 的另一独立 ledger。两份 ledger 均须至少连续 3 次同一规范签名的 `syncStatus=idle`，且顶层和逐样本 `read_error_count=0`、`retry_error_count=0`；每份 ledger 必须满足 `started_at <= observations[*].captured_at <= ended_at`，样本时间非递减。单次 `syncing` 不能立即定性为证据失败，读取错误、renderer 重试、ABA 签名漂移、时间越界/倒序或样本不足则必须 fail-closed。动作前 ledger 的 `action_evidence` 必须为 `null`，动作后 ledger 必须精确绑定本次唯一动作文件。
- 两份 catalog 的 installed identity 使用 `sourcePlatform/namespace/slug/installedVersion|version|revision|packageDigest|fingerprint`；前后集合必须非空、各自唯一并完全相等。身份字段缺失、重复、新增、丢失或集合不等均不能形成可信通过；证据结构不完整属于 `framework_issue`，真实动作及取证完整但产品新增/丢失 identity 或任一 Skill 为 `unready/python_runtime_failed`、半安装、错误状态时属于产品 Bug。每个已安装项都必须进入明确 ready，禁止 `installStatus=ok` 与失败 readiness 并存。
- Case 必须生成唯一 manifest 角色 `product_action_trace`，其专用 schema 固定为 `qbot-core-beta-skill-reinstall-product-action-trace/v1`，并精确绑定 `case_id=BETA-INIT-003`、`method=skillsReinstall`、`testid=assistant-skills-reinstall`、`click_count=1` 和完整破坏性确认。唯一动作 attempt 必须在真实 `button.click()` 紧前固化 `dispatched_at`；磁盘重放强制 `before ledger ended_at <= action dispatched_at <= after ledger started_at <= product action trace captured_at`。trace 必须以 Case 内普通文件记录和 bytes/SHA-256 同时引用 `action_observations`、`catalog_observations_before_action`、`catalog_observations_after_action`、`terminal_outcome.observations`、`terminal_outcome.screenshot`、`continuation_surface`；任一 schema、身份、点击数、确认、时序、引用、路径、bytes 或 SHA 漂移均为 `framework_issue`。terminal 与 catalog 产品 Oracle 必须组合判定：二者均成功才可 pass；terminal 明确 `pending=false/failed=true` 且 runtime/SDK/按钮/capabilities/页面/维护区可读、磁盘 continuation 重放 `safe=true` 时保留可信 Bug，不能被 catalog 全绿或 raw pass 覆盖。`skill_reinstall_readiness_verdict` 与 `initialization_continuation_surface` 仍是另外两个唯一必需角色，三者不得互相替代。
- 无论专项 Oracle 成功或形成证据完整的产品失败，收尾都必须真实点击【新建任务】恢复干净工作台。`qbot-core-beta-initialization-continuation-surface/v1` 必须证明同一 Case 的非空 `draftInstanceId`、`taskId=null`、`messageCount=0`、`running=false`、Skill/Connector 空数组和 Expert 为空，并固化互异的恢复前/后 PNG Case 内普通文件路径、bytes、SHA-256。发送守卫不再要求历史累计 `sendCount=0`：`send_count_before` 与 `send_count_after` 必须都是可观测的非负安全整数、严格全等，`send_count_unchanged=true`，且四个 `send_count_sources`、前后 e2e/public state 和 `continuation_state` 逐项绑定；缺失、负数、非安全整数或发生变化均 fail-closed。
- 可信复核不得相信 trace、verdict、surface、嵌入 manifest 或汇总的自报摘要：必须从可信不可变 run root 实读全部 JSON、PNG 和引用，逐级 `lstat` 拒绝符号链接、目录、Case/run root 越界、角色/inode 复用和读取期间替换，再重放双 ledger、identity 集合、动作/确认、维护终态、发送计数与恢复态 Oracle，并逐字段对账 manifest。Casebook 生成器必须把完整合同同步写入 `核心生命线门禁`、`生产灰度门禁Case`、`全量功能回归Case` 和 `证据与断言`；固定 G2 `新增MR核心冒烟` 明确不含 `BETA-INIT-003`。当前修改目标是待生成 r16；在 r16 正式生成并冻结 SHA 前，不得改变 r15 的正式入口身份。
...
360Teams 轻量 `smoke` 同样不得以“新增了非空助手节点”或“文本连续 1.2 秒未变化”冒充回复完成。它必须从当前可见停止控件和发送按钮只读确认 `running=false/sendButtonVisible=true`，明确排除“已经接住了，正在把回应接回来”“开场热身有点久，但我已经上场”等加载占位，并要求非占位正文至少连续 3 次采样且稳定满 1.2 秒后才能执行 expected 文本 Oracle；运行态缺失、仍在运行、发送按钮未恢复、只得到占位或超时均不得误报为 expected mismatch。
...
MR `!1573` 只有在 `iid=1573` 且 `merge_commit_sha` 精确等于
`6d482c9ccbceb74d4ebf81610d980e5fe15def6c` 时，才允许覆盖通用关键词映射并固定同序 11 条
直接 Case：`SIT-MEM-001`、`BETA-CHAT-001`、`BETA-CHAT-002`、`BETA-CHAT-009`、
`BETA-SEC-002`、`BETA-MCP-001`、`BETA-MCP-002`、`BETA-HOST-003`、
`BETA-INIT-001`、`BETA-ROUTE-001`、`MRSMOKE-ROUTE-001`，所需阶段必须精确为
`G1/G2/G3/G4`。错误 IID、错误 merge SHA 或仅命中 memory/Profile 关键词不得取得该专用
映射。验证器必须从冻结 MR 身份重算该 impact；删除 `SIT-MEM-001`、删除 `G4` 或改写汇总
...
`qbot-release-intake/1.9.0` 绑定。审计器必须先词法剔除注释、模板和正则正文，
...
所有进入原生 `setTimeout` 或受保护 `schedule` 的可配置延迟都必须遵循同一 Node timer
安全合同：来源只求值一次并转为 `requested`，仅当
`Number.isSafeInteger(requested) && requested > 0` 时才接受，否则使用固定正安全整数
fallback；随后以 `Math.min(requested, 2147483647)` 形成不可遮蔽、不可重绑的 bounded
binding。若 sink 使用 `bounded + 1`，requested 上限必须改为 `2147483646`；context lease
还须保持其更严格的 11000ms 业务上限。cancellation、deadline、event-flow、manager、
termination、context lease、supervisor startup/restart 与 process-lifecycle
heartbeat/confirmation 的 timer/envelope/比较/renew 必须使用同一 bounded 值，且 binding
在 sink 前可达并支配它。`Infinity`、超上限整数、`Number.MAX_SAFE_INTEGER`、小数、负数、
`NaN`、原始值直达 sink、未使用的伪归一化，以及 legacy/current `OR` 的任一弱分支都不得
取得 `VERIFIED`。
...
`validateExecutionWorkerReply(raw, pending, now)`。委托形态必须原样传入当前 raw、同一
pending registry 与受保护 clock；错误模块、错误 registry、调用 `now()` 后传值或局部遮蔽均须
fail-closed，且 deadline AST 子合同仍必须独立为 true，不能用委托调用替代 helper 本体鉴证。
解码拒绝分支中的诊断日志必须隔离异常，并始终对同一 `child` 调用受管
`terminateChild(child, 'message-validation-failed')`：可使用日志 `try/finally` 保证终止，
也可先以独立 `try/catch` 吞掉日志异常，再 `void` 调用或返回受管终止 Promise。直接
`child.kill()`/可选链 kill、未隔离且可中断终止的日志、错误 child、动态/错误 reason、
不可达终止，以及 `logger`/`error`/`child`/`terminateChild` 的遮蔽或重绑均须 fail-closed。
...
`createExecutionWorkerContextUsageLease`。该委派链属于 v5 当前的 16 个受保护源码文件合同；
相对 v4 的 9 个文件，当前 v5 还纳入 `execution-worker-controller.cjs`、
`execution-worker-cancellation.cjs`、`execution-worker-deadline.cjs`、
`execution-worker-callback-settlement.cjs`、`execution-worker-event-flow.cjs`、
`execution-worker-supervisor-message.cjs` 和 `execution-worker-termination.cjs`。实现必须对已完成 lease awaited 执行 `drain(...)`，
...
解构写入不得遮蔽 helper/completed，release 路径不得强制改写。context helper AST 必须始终
解析 GitLab API 返回的完整真实源码字节；空文件、测试占位源码或以 `// observed` 等注释
开头的源码都不得跳过 AST 合同，合法注释后仍须对后续完整程序执行同一严格验证。
`successor_ast_contracts` 必须精确包含 12 个子项：`cancellation`、`controller`、
`deadline`、`callback_settlement`、`event_flow`、`desktop`、`manager`、
`manager_pressure`、`supervisor`、`supervisor_exit`、`supervisor_message`、
`termination`。任一子项为 `false`
时，主 blocking-risk 审计必须保持 `verified=false/status=BLOCKED`，不得生成 `VERIFIED`。
`qbot-release-intake/1.8.0` 及更旧 intake tool version、阻断风险 v2/v3/v4 证明或任一
...
`last_commit_id` 为唯一 provenance 锚点，再通过只读
`repository/commits/<last_commit_id>` 读取提交 metadata，并完整分页读取
`repository/commits/<last_commit_id>/diff?per_page=100&page=N`，直到出现少于 100 项的
终止页。`qbot-qwork-release-file-provenance/v2` 必须固化 commit/diff endpoint、完整
canonical 提交 metadata 及其 SHA-256、全部有序分页、每页完整 `raw_response` 及其
SHA-256，并从原始响应重新生成 exact-key 的五字段 change 投影；`diff`、mode、
`generated_file/collapsed/too_large` 等非投影字段仍必须保留在原始响应并参与页哈希。
原始 change 必须具备当前 GitLab 的 `old_path/new_path/a_mode/b_mode/diff` 与三个 flags 共
8 个必需字段；仅允许额外出现上述三个已知布尔扩展字段，缺失必需字段或未知字段均阻断。
commit metadata 至少必须包含并校验 id/short id/父提交、title/message、author/committer、
authored/committed/created 时间、trailers 和规范项目 commit URL，删减后重算哈希仍须阻断。
...
每条 diff 的 `old_path/new_path` 必须为无首尾空白的非空字符串；相同 old/new 路径对即使
flags 不同也属于跨页重复，必须阻断。三个 change flag 必须是原始布尔值且与 rename 路径语义一致，
禁止把缺失或字符串 flag 静默归一化为 `false`。完整 diff 还须重新投影
至少一条
`old_path` 或 `new_path` 精确命中受保护路径的 change；该投影必须恰好一条、
`new_path` 仍精确等于当前受保护路径且 `deleted_file=false`，重复命中、删除态或
rename-away 均不得作为当前文件来源。提交 identity、页序、终止条件、change flags、
路径命中和投影必须逐项重放。禁止继续使用
`repository/commits?path=...`，也禁止只信 Files API 的 `last_commit_id` 格式、首个 diff
页或自报 `path_verified=true`。缺少 provenance、合法 SHA 改写、endpoint、分页、路径命中
或字段漂移均在 G0 阻断。
...
都必须被拒绝。`eval`/`Function` 及派生执行值经过 conditional、logical、assignment、
sequence、await/yield 或解构默认值传播后仍必须保持 taint，不能因表达式包装而恢复可信。
报告必须精确为 `dynamic_code_execution_count=0`、
...
MR !1597 的 facade 导出链允许一次包含多个静态 `require(...)` source 的
`Object.assign(exports, ...)`，但 supervisor source 必须唯一且位于最后，防止后续 source
覆盖 `workerEnvironment`。`exports`、`module.exports`、静态/动态计算属性及其直接或解构
别名都属于受保护导出 receiver；`module` 自身的别名、容器/默认值/嵌套/赋值式解构别名和
整体 `module.exports` pattern 目标同样受保护；成员赋值、update、delete、循环写入，以及
`Object.assign/defineProperty/defineProperties`、`Reflect.set/defineProperty/deleteProperty`
间接改写必须 fail-closed，包括 builtin 对象及其写函数被取别名后调用，也包括这些写函数
通过 `.call(...)` 或 `.apply(...)` 间接调用；审计必须按实际转发实参解析 target，`.apply`
参数列表不能静态解析或 target 来自 spread 时必须 fail-closed。受保护 test binding
的数组/对象/默认值解构别名也必须传播到成员写审计。`node:vm` 的 default/named-default/
namespace/named import、对象 spread/Object.assign 复制、成员取值、对象/解构别名或 `.bind()`
派生执行值，只要在文件任意位置实际调用或构造就必须计入动态执行并阻断；仅导入且从未执行
的 `node:vm` 不应误阻断。
...
状态机 `init` 无条件要求并分别封印 `--release-intake-g1`、
`--release-intake-g2`、`--release-intake-g3`、`--release-intake-g4` 四份报告的路径、
文件 SHA 与内容身份，不能通过省略参数或 `--require-release-intake false` 关闭；四份报告
必须分别绑定对应阶段的 Sheet 和有序 Case ID，路径互异且 release ref、HEAD、仓库和设计
基线全等。同时必须显式接收独立的
...
计划中的每阶段 intake 绑定必须具有正确 schema、绝对路径、
