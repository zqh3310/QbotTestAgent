...
- `QWD-ENTRY-002` 在建立任务 A 前切换 Skill/Connector 手动模式；任一真实可见控件已定位且点击已派发、但产品状态未生效时，`resetComposerControls` 必须在后续同级能力操作覆盖共享 artifact 前，把失败交互保存到 `failed_interactions`。若截图、空任务、零消息、发送计数不变和空能力选择均完整，runner 必须物化发送前产品失败 blocker，把未发生的 task/prompt/send/transcript/reply/capability execution 角色标为受校验 N/A，并为 `qwork_daily_readback`、`composer_attachment_state`、`data_integrity_readback` 写入 `evidence_valid=true/oracle_valid=false` 的负向证据。manifest 必须完整、结果保持 `bug` 并继续后续独立父 Case；失败交互、零发送读回或截图任一缺失仍是 `automation_error`。
