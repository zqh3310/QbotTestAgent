...
- 360Teams 附件拒绝产生原生 `AXSheet` 时，Core Beta v2 必须把 Playwright `dialog` 文案与 macOS Accessibility 文案精确绑定，只允许对“附件类型不支持/上传失败/数量或大小超限”白名单信息弹窗点击唯一的 `OK`、`确定` 或 `知道了`。每次必须保存派发前截图、可用时的原生截图、结构化 AXSheet 文案/按钮/点击账本、关闭后截图，并验证按钮已点击、AXSheet 已消失、页面恢复响应和 Composer 附件为空；多按钮、破坏性、无关或文案不一致弹窗必须 fail-closed。每条 Case 和 pipeline 派发开始前必须先按同一白名单清理上轮残留 AXSheet，再做 Escape/DOM 清理；不得依赖人工点击或 Computer Use。
