...
- `BETA-FILE-001` 的结构化表格若有明确“页码/Page”列，三条独立结论分别标注 `P1`、`Page 1` 或 `第 1 页`，也属于逐条页码绑定；少于三条或任一结论标到其它页仍必须失败。Agent 先记录某个附件适配器失败、随后明确完成 fallback 解析并给出正确 fixture 内容时，不得用跨句宽松正则把“改用附件工具…附件引用”拼接成“未收到附件”；只有连贯、明确的未读取/未收到/需重传终态才可触发附件丢失 Oracle。
