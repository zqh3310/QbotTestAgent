...
- 基于框架 `118e024015ef1dc3068fd692f928e2c779ce95cb`、旧 Casebook SHA
  ...
  `teams360-automation/output/20260827101200_sit-qwork-mrsmoke12_teams360-5.5.13-2119081949_qwork-0.1.6-sit.6_M3_serial_framework-35cf3d6_casebook-d09e029`。
  已真实完成 `9/12`，`inherited=0/synthetic=0`，raw 为 `passed=5/failed=4`；第 10 条
  `MRSMOKE-ART-001` 未计入 completed。停止原因不是 runner 卡死，而是 Case 开始前公开
  `runtimeReleaseStatus()` 读到冻结 `.6` 上的 `updatePhase=restart-required`、
  `preparedRelease=0.1.6-sit.7`、候选产品行为 commit `8f001d12`。框架保持更新提示原样、
  没有点击“稍后/立即重启”，写入完整 `framework-stop-diagnostic.json` 后退出；旧目录
  不得续写。已完成的 `MRSMOKE-AUTH-001` 生成有效
  `qbot-workspace-rejected-send-receipt/v1`，证明 35cf3d6 自愈路径生效；产品没有返回
  结构化 `cwd_missing`，仍是产品 Bug 候选。`MRSMOKE-ACT-001`、
  `MRSMOKE-FAIL-001` 的产品负向结果和其余已完成 Case 必须逐 Case 可信复核保留。

- 最新正式 Casebook 已更新为
  `PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-05-r15.xlsx`，
  SHA-256 `8523a10715a384f0d321f468a5350b393f19832008f585731fe83e292982ff2a`，设计基线
  `origin/release/0.1@6d482c9ccbceb74d4ebf81610d980e5fe15def6c`、产品版本 `0.1.7`。
  r15 从 r12 产品基线 `4693c5bd57b1170bed530e7559f9dc93a0b4a492` 全量重扫，继承
  r12 已验证的 134 个直接合入 MR，并通过 GitLab API freshness 按 first-parent 新增验证
  37 个直接合入 MR；本轮共 37 个增量 MR、171 个总 MR。生成 intake 为
  `outputs/20260905161230_release01-r12-to-latest-r15-intake_framework-e3ed6d5_casebook-da9181f/release-intake.json`，
  文件 SHA-256 `13bd0f57bbf6b07720b470feb708865ea7f60c75f6d55ae98a47a15940599a09`；
  构建目录为
  `outputs/20260905162000_release01-r15-casebook-build_framework-e3ed6d5_intake-13bd0f57`。
  构建审计为 `qbot-release01-combined-casebook-build/v8`，14 个 Sheet、15 张渲染、公式
  错误 0，四层导出后能力审计分别为 16/16、12/12、70/70、160/160，G4 前 70 条与
  G3 精确同序，`strict_controller_required=0`、`unsupported_runtime=0`。

  r14 历史基线为 `origin/release/0.1@0cfdfa1ec9f18d2ef2e78d380b4b2896c6dc607c`；
  它继承 r12 已验证的 134 个直接合入 MR，并新增验证 36 个直接合入 MR，共审计 170 个 MR。
  r13 候选 `QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-04-r13.xlsx`
  因 `MRSMOKE-WEB-001` 四轮 prompt/Oracle 不完整、`BETA-TASK-002.source_id` 重复，且
  未在正式复制前强制四个 Sheet 的 protocol/runtime 审计而判定 `FAILED`；该文件只作
  历史失败证据，不得作为正式入口。r14 已修复三项问题，导出后四层审计均为
  16/16、12/12、70/70、160/160，且正式复制前 fail-closed；r15 不覆盖这些历史记录。
...
- r15 生成输入已从 r12 基线使用 `--max-commits 500` 完成全量扫描；构建审计
  ...
  定义 `execution_authorized`。提交推送并确认最终 `main == origin/main`、tracked clean 后，
  必须用最终框架 commit 对最新 release/0.1 重做 GitLab API 扫描；仅新执行 intake、独立
  ...
  通过不得冒充这些内部合同已执行或通过。历史空目录
  `outputs/20260905_release01_casebook_16-12-70-160-r15` 未被复用；正式构建使用上述新目录。
  生成器不得覆盖既有正式目标，也不得在四个执行 Sheet 的 16/16、12/12、70/70、160/160
  导出后 protocol/runtime 审计全绿前发布到 `PRD`。生成器必须在 `--out` 同父目录的当前用户私有
  ...
  symlink/hardlink 或故障注入均须 fail-closed，且不得留下未经验证的正式目标。
...
```text
/Users/qifu/Documents/QbotTestAgent/PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-05-r15.xlsx
SHA-256: 8523a10715a384f0d321f468a5350b393f19832008f585731fe83e292982ff2a
```

r15 已由生成器排他创建，独立验收和四层能力审计通过；本节、主框架手册、状态机正式
文件名/SHA 和 invariant 在同一提交中切换。正式执行仍必须等待该提交推送并确认
...
deepbankV2 GitLab API 连续两次读取稳定 HEAD；后续状态机命令继续携带
...
都复核计划、状态、integrity、事件数量/顺序/链和末事件 SHA。初始状态必须由计划确定性
重建并以 `initial_state_sha256` 锚定；事件必须保存完整 `state_before/state_after`，并用
原始 `audit/phase/recorded_at` 重放转换后与快照及链尾状态全等；任一文件或历史事件被
改写立即 fail-closed。`init` 必须在同父目录私有 staging 完整 fsync 后原子 rename；阶段
更新先保存 write-ahead transaction，再写 event/state/integrity；进程中断后只允许回滚未提交事件，或以严格绑定的尾事件幂等
补齐 state/integrity。独立 release observation 必须先证明规范 deepbankV2 origin，再实时
查询 release ref：Git credential 路径要求 remote/观测/本地 remote-tracking ref 全等；
stdin API 路径固定 GitLab 主机与项目、强制 HTTPS/TLS，并要求两次稳定 branch read 与
观测 HEAD 全等。仅改 URL 后沿用旧观测不得通过。`NO_GO` 不可逆，已有 readiness/completion 不允许覆盖；恢复测试
必须新建空控制目录，禁止编辑 JSON 伪造准入。
独立观测的输出目录必须在调用前不存在，不能复用空目录；写入前后必须证明全部祖先无
符号链接、父目录和目标目录属于当前用户且不可被 group/other 写入，并锁定两级目录的
dev/inode/uid/mode。观测文件必须以 `O_EXCL/O_NOFOLLOW`、私有权限和 fsync 落盘，再核对
普通文件类型、字节数与权限；任一替换或漂移均 fail-closed。
...
越界，再以实际文件字节数和 SHA-256 对账，不能只信 manifest 声明。run/cases/Case/
manifest/证据路径还要逐级拒绝符号链接，并在 realpath 后继续位于 run 根。可信复核必须
是 run 根内普通文件并把实读 SHA 写入 completion audit；每项必须显式 `trusted=true`
且显式分类为 `trusted_pass` 或 `可信通过-用户可接受`，缺字段不能由汇总计数补足。Teams 的
...
每次 `readiness` 在写入事件前必须重算 Casebook 磁盘 SHA，并验证当前
`branch=main`、`HEAD == origin/main == plan.framework.commit`、tracked clean；同时重读
独立 release HEAD 观测并校验文件 SHA。失败时 revision、integrity 和 events 均不得变化。
G5 仅接受磁盘 `qbot-qwork-soak-report/v1` 普通文件及其实读 SHA；报告逐项列出至少 100
个唯一真实 task、至少 3 次受管重启、`startup/run-final` 与每次重启后的十字段身份观测、
显式空 crash 账本和带阈值/多点采样的无泄漏账本。每个 task、restart、identity observation
和资源账本都必须绑定目录内唯一普通证据文件，并逐祖先/realpath/bytes/SHA 校验；自报
计数、布尔值或单一 identity hash 不构成 G5 证据。
...
- 自动化覆盖活动流、Web 搜索与 SSRF、目录授权、interval 真实到点、侧栏布局、
  ...
  不在 12 条通过结论中豁免。
- r14 的冻结父输入仍是
  `PRD/QBot核心生命线与新增MR生产灰度全量回归Casebook_16-12-70-160条_2026-09-03-r12.xlsx`，
  SHA-256 `da9181fdc4e8d63ec5e9ed1bad231b4ffe78870b085d96598586070b97cf8c54`；该历史父输入只用于
  证明 134 个既有 MR 的继承链，不得作为当前正式执行入口。
- r12 以 GitLab API freshness 从 r11 设计基线
...
- r12 继承 r11 已验证的 132 个 MR，并新增 2 个直接合入 MR `!1523`、`!1522`，共审计
  ...
  !1460、!1480、!1477、!1484、!1481、!1494、!1474、!1488、!1495、!1496、!1491、
  !1492、!1498、!1499、!1485、!1501、!1479、!1490、!1486、!1489、!1504、!1506、
  !1483、!1487、!1497、!1508、!1512、!1514、!1515、!1509、!1503、!1513、!1518、
  !1519、!1524、!1521、!1520。
- r8 历史覆盖中包含 !1334、!1331、!1330、!1332、!1336、!1337、!1338、!1310、
  !1339、!1340、!1333、!1341、!1326、!1342、!1320、!1344、!1343、!1345、!1346、
  !1349、!1348、!1350、!1355、!1354、!1356、!1357、!1352、!1359、!1361、!1364、
  !1358、!1365、!1428、!1430、!1443、!1450、!1451、!1374；其中 `!1374` 的 Auto
  fallback/catalog authority 变更映射 `MRSMOKE-ROUTE-001`、`MRSMOKE-FAIL-001`、
  `BETA-CHAT-005` 与 `BETA-PERF-003`。
- 最新 `!1520` 的 SDK 解压让出事件循环变更映射 `BETA-INIT-001/003`、
  `BETA-HOST-003`、`MRSMOKE-NAV-001` 与 `SIT-TEAMS-NEW-001/003`，核对运行时物化期间
  Teams Tab 切换和宿主身份不阻塞。
- 最新 `!1516` 的 VPN 未连接明确提示与 retry 埋点变更精确映射
...
- 最新 `!1523` 的 Web 搜索固定次数误导修复精确映射 `MRSMOKE-WEB-001`、
  `MRSMOKE-WEB-002`、`BETA-CHAT-005` 与 `SIT-CONN-019`；只有前者执行同一 task 四轮
  真实 provider 调用，`SIT-CONN-019` 保持单轮质量验证，禁止按 web 文件名启发式泛化。
- 最新 `!1522` 的 Claude Code SDK Header 注入变更精确映射 `MRSMOKE-ROUTE-001`、
  `BETA-CHAT-001`、`BETA-ROUTE-001` 与 `BETA-HOST-003`。Header 字节只由源码静态合同
  审计证明；桌面 E2E 只验证同任务多轮、fallback 连续性和宿主/runtime 稳定性，不声称
  UI 或桌面证据已证明 Header 注入，禁止按 runtime 文件名启发式泛化。
- `!1522` 还必须通过 intake 源码合同 `deepbankv2-mr-1522-claude-turn-headers/v1`：merge SHA
  固定为 `4693c5bd57b1170bed530e7559f9dc93a0b4a492`，GitLab changes 恰为 5 项，规范化完整
  MR diff 为 18038 bytes、SHA-256
  `f1a9b0af3a286e55add0af61b7703af6f85a003d955ab7b6cdbe4704a6de4c80`；新文件
  `server/qbot-core/models/claude-turn-headers.mjs` 必须从 diff 无损重建为 3673 bytes、
  111 行、SHA-256 `e81904c2527675117a74d8227b1ee2761bfeb59093c7a4b65c63c4d4f5fcd62d`。
  静态审计逐行唯一证明 `User-Agent`、`x-session-id`、`x-turn-id`、`x-request-id`、
  `x-request-time` 的精确 value source，以及宿主 turnId、引擎注入和 fallback 的 turnId/
  request context 接线。attestation 自身 SHA、MR 双向绑定和 summary 计数必须完整；任一
  merge/diff/源码/Header/接线/绑定/计数不一致，或 attestation 缺失、重复、伪造，都必须
  进入 unresolved 并令 G0 `BLOCKED`，不得由桌面 Case 通过替代。
- MR !1329、!1330、!1337、!1310、!1340、!1333、!1326、!1342、!1344 继续固定为
  ...
  其余产品行为变化映射到现有真实 Case，完整映射以工作簿 `近2天MR覆盖` Sheet 为准。
- `MRSMOKE-AUTO-001` 固定 `intervalMs=60000`、`activeFrom=当前时刻`，并覆盖 !1357 的后台执行隔离，禁止
...
- `MRSMOKE-WEB-001` 必须在同一非空 task 连续四轮确认发送，每轮绑定唯一 prompt SHA、
  runtime authority、materialized `builtin:qbot_web`、provider receipt 和截图，并生成
  `web_search_quota_trace`；四个 provider receipt 必须有效且唯一，第四轮仍须真实调用且
  不得出现“最多三次”“额度用尽”“固定上限”“服务端拒绝”或
  `You can only search three times`、`The server rejected the fourth search`、
  `The search quota has been hit` 等同义误导。`SIT-CONN-019` 保持单轮。
  manifest 必须从权威 Case 目录重读 trace，重算每轮 trim 后 prompt SHA，并对四张唯一
  截图执行 lexical + realpath 边界、`lstat` 普通文件、非 symlink、最小字节和实际
  bytes/SHA-256 对账；非 JSON trace、目录、路径逃逸或任一自报元数据不一致均须
  fail-closed。`evidence_valid=true/oracle_valid=false` 的完整产品拒绝证据仍保持有效。
...
- 160 条全量：61 条原生/public-state、1 条原生 IME 选项、98 条经过语义复核的 legacy executor；合计 61/1/98。
...
Casebook 的设计依据是 r11 已验证的 132 个 MR，以及从
`1970fe47ac681b36242b0be5c4175238f7d9918b` 到
`4693c5bd57b1170bed530e7559f9dc93a0b4a492` 由 GitLab API freshness 完整验证的
2 个增量 MR，共 134 个。r11 仍完整保留 r10 的 131 个 MR、r10 的 `!1516`、r9 的
57 个增量 MR 历史序列与 r8 的 73 个 MR 继承关系。
...
覆盖矩阵、执行配置和发布准入均在工作簿独立 Sheet 中；静态合同审计不得冒充桌面 E2E。
...
G5 不接受汇总对象中的自报布尔值或计数。唯一合法输入是新不可变目录中的磁盘
`qbot-qwork-soak-report/v1` 及其实读 SHA；报告和全部证据必须位于同一目录树，均为
互不复用、非符号链接的普通 JSON 文件，并逐项校验 owner/role、realpath、inode、bytes
和 SHA-256。每个 task 必须绑定唯一 prompt/marker、task/session、host/renderer context、
dispatch/严格发送/成功终态三类 receipt 和至少三次稳定终态观察，且 100 个以上 task
全局串行、全部真实执行、无 inherited/synthetic。

至少 3 次受管重启都必须替换 host/renderer PID 与 process-start、session、CDP 和 WebView
target 的完整 tuple，并由精确的 `restart-before/restart-after` 十字段身份读回闭合；观察
集合只允许 `startup + 每次重启前后 + run-final`，且首尾及重启边界观察不得陈旧超过策略
最大采样间隔。crash ledger 连续覆盖每个宿主 epoch并证明零异常；资源账本按每个 epoch 的
host/renderer 进程身份至少各采样两次，声明和实际采样间隔均不得超过上限。校验器从全部
样本重算中途 RSS peak、相对 epoch 首样本的最大 growth 和相邻样本最大 slope，三项均须
低于阈值；仅首尾 RSS 正常或 `no_leak=true` 不能放行。

G1-G4 全部由同一状态机可信通过后，先生成新的真实 G5 报告。Token 必须只通过关闭回显
的标准输入临时注入本次只读状态复核：
...
G1-G4 Casebook 与 G5 Soak 直接入口共用
`teams360-automation/runtime/.qwork-managed-runner.lock` 的 macOS `lockf` 生命周期锁，竞争时
只允许一个 runner；锁文件本身被精确忽略，但不得忽略整个 `runtime/`。G5 输出目录必须
此前不存在、位于受管输出根内且全链路无符号链接。报告生成并自审通过后，执行：
...
只有磁盘复核生成的 `qbot-qwork-soak-completion-audit/v1` 为 `PASS_STAGE` 才完成 G5；
报告身份 SHA 与 framework commit 必须和冻结计划、本轮候选精确一致。

状态机的每轮 G3/G4 completion audit 必须同时证明：
...
多轮 `runs.json` 固定精确 5 项，每项 schema 必须为 `qbot-core-gray-run/v2`，只允许
`run_id/stage_id/control_dir/release_plan/completion_event/soak_report` 六个载荷字段；
`control_dir`、plan 和 G3/G4 completion event 都必须使用绝对规范路径与实读文件 SHA。
`run_id` 必须等于 completion evidence tree 目录名。旧式
`total/completed/trusted_counts/evidence` 自报即使全绿也必须拒绝。

聚合器从初始状态完整重放五棵 v2 event/state/hash 链，逐 event 重算外部文件和目录树
SHA，并通过 `applyQworkStageAudit()` 重新执行磁盘 `auditQworkStageCompletion()`。五轮
release identity、framework commit、正式 Casebook 路径/SHA、G3 等价 70 条 Case ID 和
完整 Case 合同必须全等，completion 时间严格递增；控制目录、completion event、全部
阶段 run directory、可信复核、证据树的路径、内容 SHA-256 或 device/inode 任一跨轮复用
都必须拒绝，五项 `run_id` 也必须唯一。至少一项
必须是真实 G4 160/160，其前 70 条同序同合同，且只计一个 70 等价轮次。

五项中精确一个 `soak_report` 可为 `{path,sha256}`，且只能位于 G4 项；同一控制树必须在
目标 G4 completion 后存在唯一 G5 `PASS_STAGE` completion event，并由该 event 绑定同一
Soak 路径/SHA、计划身份和 framework commit。旧式内嵌 Soak、自报身份、跨控制树报告、
混合 commit、重复证据或非法 3/4 轮策略统一返回结构化 `NO_GO/STOP_PIPELINE`，不得抛出
未捕获异常。
...
npm run core-beta:pretest -- \
  ...
  --release-intake "$PWD/outputs/<new-immutable-release-intake-dir>/release-intake.json" \
...
- `BETA-EXPERT-012` 维护回复必须先显式满足 `reply_incomplete=false` 且
  `timeout_cleanup_ok=true`，随后才允许读取草稿、打开完整配置或发布。回复未终态、字段
  缺失或清理失败时立即记录 automation error 并终止该路径，严禁在 Agent 仍运行时继续
  产生真实 version/release。
...
current-release Files API 返回的受保护文件必须把 `blob_id` 精确绑定到内容字节：验证器
按 `sha1("blob <byte-length>\\0" + content-bytes)` 重算 Git blob SHA-1 并全等比较；40 位
格式本身不能放行。Casebook 必须为可读的普通非符号链接文件，必须显式提供存在的 Sheet，
导出结果至少包含一个唯一 Case ID，声明 SHA-256 与磁盘实际字节全等。文件、Sheet、导出、
唯一性或 SHA 任一失败都令 intake `BLOCKED`，禁止吞错后以空 Case 集继续。

从 `qbot-release-intake/1.5.0` 起，compare 中每个 first-parent commit 都必须进入
`commit_accounting`。多父提交只接受精确 `merge_commit_sha`，单父提交只接受精确
`squash_commit_sha`；两类均要求唯一 merged MR、目标分支全等和完整 changes。无法可信
归因的单父提交写入 `unattributed_direct_commits` 并阻断。freshness 中的 first-parent、
accounted、merge、squash 和 direct 五类计数必须与逐提交账本闭合，验证器独立重算，不能
通过修改顶层 `verified` 或重算内容 SHA 绕过。
MR 风险生效点和后继架构切换点必须各做正向、反向两次 first-parent compare。只有反向
...
SHA，也不能靠改写 MR 行、覆盖映射或汇总字段取得 `READY`。正式 token 只能发往
`https://gitlab.daikuan.qihoo.net` 的 `songrongxin/deepbankv2` API，TLS 证书校验和
HTTPS 限制必须开启；`insecure/-k`、自定义主机或自定义项目均不得用于正式 freshness。

MR `!1573` 的专用 impact 必须同时精确绑定 `iid=1573` 与 merge SHA
`6d482c9ccbceb74d4ebf81610d980e5fe15def6c`，并覆盖通用关键词映射，固定同序 11 条直接
Case：`SIT-MEM-001`、`BETA-CHAT-001`、`BETA-CHAT-002`、`BETA-CHAT-009`、
`BETA-SEC-002`、`BETA-MCP-001`、`BETA-MCP-002`、`BETA-HOST-003`、
`BETA-INIT-001`、`BETA-ROUTE-001`、`MRSMOKE-ROUTE-001`；required stages 必须精确为
`G1/G2/G3/G4`。错误 IID、错误 merge SHA 或仅有 memory/Profile 关键词都不得取得专用
映射。验证器必须重算该身份和 impact；删除 `SIT-MEM-001`、删除 `G4` 或改写汇总后，
...
静态路径和产品源码路径必须分别命中显式白名单后才可分类；陌生非静态路径不参与关键词
映射。文件名、标题、分支或标签包含已知领域词不能把陌生路径转成已有 Case 覆盖。
源码合同鉴证分为两层，必须独立命名、计数。每次正式 GitLab API 扫描都要为注册表中的
全部合同生成 current-release 持续性鉴证：从当前 release HEAD 重新读取受保护文件，并
证明该 HEAD 对合同 origin merge 的完整祖先关系；它只证明当前 release 仍保有声明的源码
与测试声明，不能冒充本轮 MR changes 鉴证或测试执行结果。只有扫描范围命中已登记合同的
...
正式 pretest 的 `--production-gate true` 默认要求 `--release-intake`；正式
production-gate pretest 自身不可关闭 intake，且必须显式提供有效的 64 位
