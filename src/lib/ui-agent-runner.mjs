...
function generateUiAgentFixtures({ fixturesDir }) {
  ...
  writeXlsx(path.join(fixturesDir, 'qbot-data-table-diff.xlsx'), {
    registered: 120,
    arrived: 80,
    sold: 15,
  });
  files.push(path.join(fixturesDir, 'qbot-data-table-diff.xlsx'));
...
function writeXlsx(file, values = { registered: 100, arrived: 70, sold: 12 }) {
  writeStoredZip(file, {
    ...
    'xl/worksheets/sheet1.xml': `<?xml version="1.0" encoding="UTF-8"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData><row r="1"><c r="A1" t="inlineStr"><is><t>指标</t></is></c><c r="B1" t="inlineStr"><is><t>数值</t></is></c></row><row r="2"><c r="A2" t="inlineStr"><is><t>报名人数</t></is></c><c r="B2"><v>${values.registered}</v></c></row><row r="3"><c r="A3" t="inlineStr"><is><t>到场人数</t></is></c><c r="B3"><v>${values.arrived}</v></c></row><row r="4"><c r="A4" t="inlineStr"><is><t>成交单数</t></is></c><c r="B4"><v>${values.sold}</v></c></row></sheetData></worksheet>`,
  });
}
